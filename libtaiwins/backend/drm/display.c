...
static inline int
crtc_idx_from_id(struct tw_drm_backend *drm, int id)
{
	for (int i = 0; i < 32; i++)
		if (((1 << i) & drm->crtc_mask) && id == drm->crtcs[i].id)
			return i;
	assert(0);
	return -1;
}
...
crtc_from_id(struct tw_drm_backend *drm, int id)
{
	for (int i = 0; i < 32; i++)
		if (((1 << i) & drm->crtc_mask) && id == drm->crtcs[i].id)
			return &drm->crtcs[i];
...
find_plane(struct tw_drm_display *output, enum tw_drm_plane_type t)
{
	...
	int crtc_idx = crtc_idx_from_id(drm, output->status.crtc_id);

        wl_list_for_each(p, &drm->plane_list, base.link)
...
find_display_crtc(struct tw_drm_display *output)
{
	...
	struct tw_drm_crtc *crtc, *potential_crtc =
		crtc_from_id(drm, output->status.crtc_id);
	...
	if (potential_crtc && (!potential_crtc->display)) {
		...
		wl_list_for_each(crtc, &drm->crtc_list, link) {
