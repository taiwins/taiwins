...
*** TODO taiwins close, reload functions.
*** TODO setup the xkb context and all that stuff
** TODO [66%] Configurations
*** TODO crazy idea. Implement client side configuration into protocols? So I can set the configure on the fly?
    This would be done in tw_ui, for example, we can have a command to tell the
    shell and console to reload its configuration from the file.
...
** TODO [82%] client-side optimization
*** TODO resize window through idle task in the event queue.
