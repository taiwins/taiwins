...
* TODO [37%] Stage three: extensions/ post-pub features
** PEND ADD DAMAGE SUPPORT in drawing!
   for EGL backend, this would be impossible, but maybe you can do something in
   the cairo backend
** TODO [83%] tw_theme protocol
*** DONE creating the taiwins_theme header
*** DONE tw_theme protocol
    Well, I should have documented the tw_theme work done before.
*** DONE tw_theme to nk_style
*** DONE lua_theme to tw_theme
*** TODO test theme with images, not just colors
*** CANC c files
** TODO [75%] console improvements
   Now the launcher, or we called it console is able to start some application,
   but it is really ugly, does not support any keyboard based navigations. 
   So there are few things left:
*** DONE navigation support, tab/enter control
*** DONE resizing.
*** DONE better drawing.
*** DONE loading also the icons(since you had icon caches).
*** TODO configuration and plugins. Yes, of course you need plugins.
*** CANC nku_branch (widget helper)
    I did not need it in the end. Maybe writing a nku branch would be a good
    idea in the end, but creating it just for one app is not sufficient.
*** DONE load only necessary icons
...
** CANC patch 3rdparty libraries
   weston: output configure in the run time.
** TODO [37%] new features
*** DOIN add dbus support
    now we have the dbus code, we can move some of the client requests into
    dbus. But at the same time, for it to have users, we would need to have lua
    bindings.
**** DONE tdbus 3rdparties
     right now we wrapped dbus with functions like message, signal, method,
     watcher. I don't think there is more to come
**** DONE tdbus added in client
**** DONE tdbus added in server
**** TODO tdbus lua callback
     I shall come back later for this.

     The major functions here are just sending and listening on signals. But
     dbus interface is tricky, leading composary args followed by a variable
     length of args. so I think the interface would look like this:
#+BEGIN_SRC lua

busAPI = require('tw-bus')
session_bus = busAPI:get_bus('session')
system_bus = busAPI:get_bus('system')

-- sample code for method call
message = session_bus:call( function () return 'aaa' end,  -- the reply method
{
.path = '/org/taiwins',
.interface = 'org.taiwins',
.method = 'hello',
.signature = 's',
}, 'the message to send')

session_bus.send(message)

-- sample code for signal
session_bus:signal_on(function (vol) w:volume = vol end, 
{
.path = '/org/taiwins',
.interface = 'org.taiwins',
.signal = 'some_signal',
...
-- it is either this API or reversing function and the table

#+END_SRC
**** TODO tdbus functions in server
     this would take a list and continuing effort for adding new methods.

*** CANC move workspace_add_view in the commit event.
    At that time, you would know about view geometry, it would be better for
    floating layer.

    This cannot be done, as commit can happen multiple times.
*** TODO set animations?
...
**** TODO expose drm output, not just windowed output for configuration
**** TODO dynamic changing output scale

*** TODO utilise weston-animations
    fade/zoom/move, it should be relative easy to use.

*** DONE adding support for notification. All those can be implement
*** DONE adding support for menu(right-click)
