...
**** TODO tdbus lua callback
     This is tricky, it would be a bit much to add lua code in the tdbus, if you
     want to do it properly. On the other end, writing just a wrapper on the
     shell would be easy, but it involves having a variable arg in the
     runtime. The simplest way is actually writing a iterator since we already
     have a iterating function in tdbus.
