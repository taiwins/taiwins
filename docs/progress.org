...
* TODO [20%] Stage three: extensions/ post-pub features
** TODO ADD DAMAGE SUPPORT in drawing!
** TODO [40%] tw_theme protocol
*** DONE creating the taiwins_theme header
*** TODO tw_theme protocol
*** CANC c files, you probably need to work with Json-c
*** TODO Add multiple font support
    one icon one ascii font isn't really the best way to handle fonts(the text
    handling code I wrote just to have two fonts working is crazy). Projects
    like nerd-font is a sign that we can incorperate multiple emoji/symbols into
    text.
