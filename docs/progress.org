...
* TODO [0%] Stage two: feature completion
  ...
  - resize EGL windows.
  - compositing
** TODO [76%] server-side optimization
*** PEND setup the xkb context, again, related to [[configuration]]
    This one requires patches to weston, which I already did, but I need their
    response. If the patch gets merged. It would be fairly simple to do, right
    now I can simply write a shell function.
*** TODO multi monitor configuraiton, which also relates to [[configuration]]
*** TODO [#B] passing server events to shell, switch workspace. e.g.
    - switch workspace.
...
*** DONE [#A] hot config I :: general hook
    ...
    keys in the config, so they get notified when value changed.
     ...
     - ...
*** TODO [#B] hot config II :: move the responsibilities like theme, font, size config in the server
    font, theme, those things, like I said, can pass to clients using
    ~tw_ui.config~, or ~shell.config~, the xkb_context is also here, so you can
    send clients the proper setups.

*** TODO [#A] hot config III :: taiwins_config_lua_plugin or other better names,
...
*** DOIN [#C] implement the libweston-desktop protocol                :proto:
...
** TODO [83%] client-side optimization
*** TODO app_surface flags
*** TODO [#C] client widgets lua bindings.
