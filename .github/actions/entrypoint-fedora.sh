...
dnf install -y weston-devel libinput-devel libudev-devel libevdev-devel
dnf install -y libjpeg-turbo-devel libwebp-devel libpng-devel
dnf install -y mesa-libGL-devel mesa-libEGL-devel mesa-libGLES-devel vulkan-headers mesa-vulkan-devel mesa-vulkan-drivers
dnf install -y mesa-libgbm-devel libdrm-devel libva-devel xorg-x11-server-Xwayland libXcursor-devel
dnf install -y fontconfig fontconfig-devel freetype freetype-devel librsvg2 librsvg2-devel
dnf install -y lua-devel cairo-devel pam-devel dbus-devel pipewire-devel
